...
if (!cli.input[0]) {
  ...
  This function searches individual repos!
    ...
    .then(json => {
      if (json.message) {
        ...
        if (json.type !== 'Organization') {
          console.log(`${chalk.green('»')} ${chalk.bold('Followers/following')}: ${json.followers} / ${json.following} (${json.followers / json.following == "Infinity" ? `${json.followers / json.following} ratio (you're not supposed to see this!)` : `${json.followers / json.following} ratio`})`)
        } else {
        ...
        console.log(json.blog != '' ? `${chalk.green('»')} ${chalk.bold('Website')}: ${link(`${json.blog}`, `${json.blog}`)}` : `${chalk.red('»')} No ${chalk.red('public website')} found.`)
